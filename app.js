...
const defaultState = {
  ...
  settings:{night:false, compact:false}, activity:[]
};
let state = loadState();
function loadState(){
  const raw=safeStorage.get(); let parsed=null;
  try{parsed=raw?JSON.parse(raw):null}catch(e){parsed=null}
  if(!parsed||typeof parsed!=='object'||Array.isArray(parsed)) return {...defaultState};
  const s={...defaultState,...parsed};
  for(const k of ['errors','mocks','customWords','activity']) if(!Array.isArray(s[k])) s[k]=[];
  for(const k of ['completed','mastery','notes','vocab','vocabDefs']) if(!s[k]||typeof s[k]!=='object'||Array.isArray(s[k])) s[k]={};
  if(!s.settings||typeof s.settings!=='object'||Array.isArray(s.settings)) s.settings={...defaultState.settings};
  return s;
}
...
const DIAGNOSTIC = [
 ...
 {m:'quant',q:'A jacket marked ₹2400 is discounted 25%, then the sale price is increased by 10%. Final price?',o:['₹1800','₹1980','₹2040','₹2100'],a:1,skill:'Percent'},
 {m:'quant',q:'If x/y=3/5 and x+y=64, what is x?',o:['24','32','40','48'],a:0,skill:'Ratio'},
 {m:'quant',q:'For x>0, Quantity A: x+1/x. Quantity B: 2.',o:['A greater','B greater','Equal','Cannot be determined'],a:0,skill:'QC / algebra'},
...
function renderDashboard(){
  ...
  const exam = state.examDate ? new Date(state.examDate) : addDays(new Date(),120);
  ...
  return `<div class="page">
    <section class="hero"><div class="hero-grid"><div>
      ...
      <div class="score-card"><small>Runway</small><b>${days}</b><p>days to ${state.examDate?fmtDate(state.examDate):'default 120-day plan'}</p></div>
    </div></div></section>
    ...
    <div class="daily-plan">
      ...
      <section class="card agenda-card"><span class="eyebrow">Daily minimum</span>
        <div class="agenda-row"><span class="agenda-icon">Aa</span><div><strong>Vocabulary retrieval</strong><small>20 active recalls + 5 new words</small></div><span class="agenda-time">15m</span></div>
    ...
    <div class="stat-strip">
      ...
      <div class="stat-cell"><b>${state.xp||0}</b><span>mastery XP</span></div>
      <div class="stat-cell"><b>${state.errors.length}</b><span>errors logged</span></div>
...
function deepPoints(t){return (window.GRE_DEEP_CONTENT&&window.GRE_DEEP_CONTENT[t.id])||[]}
function topicToolkit(t){return (window.GRE_TOPIC_TOOLKIT&&window.GRE_TOPIC_TOOLKIT[t.id])||{tricks:[],formulas:[]}}
function toolkitLabels(t){
  if(t.module==='quant')return ['High-yield GRE tricks','Must-memorize formulas'];
  if(t.module==='verbal')return ['High-yield GRE tricks','Logic formulas & decision frameworks'];
  if(t.module==='writing')return ['High-yield GRE writing tricks','Essay frameworks & scoring formulas'];
  return ['High-yield execution tricks','Timing formulas & decision rules'];
}
function renderTopicToolkit(t){
...
function lessonTimeRange(t){
  ...
  const active=Math.max(26,Math.ceil(words/150)+20);
  return `${active}–${active+15}`;
}
function renderDeepDive(t){
  const pts=deepPoints(t); if(!pts.length)return '';
  return `<section class="lesson-section deep-section"><span class="eyebrow">Deep mastery notes</span><h2>The layer the shortcut notes leave out</h2><p class="section-intro">These are the details, restrictions, edge cases, and decision rules that make this chapter usable on an unfamiliar GRE problem rather than merely recognizable on a flashcard.</p><div class="deep-grid">${pts.map((p,i)=>`<article class="deep-note"><span class="deep-num">${String(i+1).padStart(2,'0')}</span><div><h3>${['Mental model','Boundary conditions','GRE translation','High-value shortcut','Trap-resistant reasoning','Transfer rule'][i]||`Mastery point ${i+1}`}</h3><p>${esc(p)}</p></div></article>`).join('')}</div></section>`
}
function attackSequence(t){
  const pts=deepPoints(t);
  const common=t.module==='quant'?[
    ['Parse the domain','Underline restrictions: integer/positive/distinct/nonzero, units, diagram conventions, and exactly what quantity is requested.'],
    ['Name the structure','Before arithmetic, identify the governing relationship and the critical cases where its behavior can change.'],
    ['Choose the cheapest valid method','Simplify, factor, estimate, test cases, backsolve, or compute—whichever proves the answer with the least fragile work.'],
    ['Audit the answer','Check sign, scale, units, endpoints, and whether another legal case could change the result.']
  ]:t.module==='verbal'?[
    ['Map the logic','Identify speaker, task, contrast/support signals, scope words, and the exact claim the question asks about.'],
    ['Predict before choices','State the needed meaning, inference, or rhetorical role in plain language before attractive options can anchor you.'],
    ['Demand textual proof','For each survivor, locate the phrase or logical relation that licenses it; reject scope, viewpoint, certainty, and causality distortions.'],
    ['Re-read with the choice inserted','For TC/SE check full-sentence coherence; for RC check that every word of the option remains within the passage’s evidence.']
  ]:t.module==='writing'?[
    ['Decode the instruction','Separate the topic from the specific task and decide what degree of agreement, conditions, or consequences you must address.'],
    ['Build the reasoning skeleton','Thesis → body claims → mechanisms → examples → counterpressure. Plan the logic before drafting sentences.'],
    ['Develop, do not list','After every example, explain why it proves the paragraph claim and how that claim advances the thesis.'],
    ['Revise for score impact','Repair task drift, logical gaps, unclear syntax, and repeated grammar errors before cosmetic wording changes.']
  ]:[
    ['Define the decision rule','Turn advice into an observable action: a skip threshold, review trigger, pacing checkpoint, or error-prevention check.'],
    ['Use timed evidence','Keep the rule only if repeated practice shows that it protects expected points or reliability.'],
    ['Track the failure mode','When the rule fails, classify whether the problem was content, reasoning, execution, timing, or emotional decision-making.'],
    ['Retest after delay','A strategy is learned only when it survives a fresh timed set without conscious improvisation.']
  ];
  if(pts[0]) common[1][1]+=` Chapter-specific anchor: ${pts[0]}`;
  return `<section class="lesson-section"><span class="eyebrow">GRE attack algorithm</span><h2>A repeatable way to execute</h2><div class="attack-sequence">${common.map((x,i)=>`<div class="attack-step"><span>${i+1}</span><div><h3>${x[0]}</h3><p>${esc(x[1])}</p></div></div>`).join('')}</div></section>`
}
function renderTopicExamples(t){
  const pts=deepPoints(t);
  if(!pts.length) return '';
  const modulePrompt=t.module==='quant'
    ? `A GRE Quant problem is really testing “${t.title},” but the surface story is unfamiliar. What should a top-score solver do before committing to arithmetic?`
    :t.module==='verbal'
    ? `A GRE Verbal item disguises “${t.title}” inside dense prose and tempting answer choices. How should you control the reasoning before selecting anything?`
    :t.module==='writing'
    ? `You receive an Issue prompt for which “${t.title}” is the main weakness in your first draft. How do you repair the response under the 30-minute constraint?`
    :`A timed section exposes a weakness in “${t.title}.” How do you turn that observation into a repeatable test-day behavior?`;
  const trapPrompt=t.module==='quant'
    ? `A student knows the formula or rule for this chapter but applies it without checking restrictions. Show the audit that prevents the common GRE miss.`
    :t.module==='verbal'
    ? `A student recognizes the topic and picks an answer that sounds plausible. Show how to force the choice to earn its place from logic/evidence instead.`
    :t.module==='writing'
    ? `A student adds length and sophisticated vocabulary but the paragraph still feels shallow. Show what analytical work is actually missing.`
    :`A student follows a strategy because it “feels right” rather than because it improves outcomes. Show the correction.`;
  const p0=pts[0]||t.summary,p1=pts[1]||'',p3=pts[3]||pts[2]||'',p5=pts[5]||pts[4]||'';
  return `<section class="lesson-section"><span class="eyebrow">Worked examples · separate from checkpoint</span><h2>See the reasoning, then solve something different</h2>
    <div class="example-box deep-example"><span class="label">Worked example A · method selection</span><h4>${esc(modulePrompt)}</h4><div class="solution">
      <div class="step"><span class="step-num">1</span><div><b>Identify what actually controls the problem.</b><br>${esc(p0)}</div></div>
      <div class="step"><span class="step-num">2</span><div><b>Check the restriction or edge case before executing.</b><br>${esc(p1)}</div></div>
      <div class="step"><span class="step-num">3</span><div><b>Convert that into an exam behavior.</b><br>State the relevant rule in one line, perform only the work needed to prove the answer, and verify the requested quantity before moving on.</div></div>
    </div></div>
    <div class="example-box deep-example"><span class="label">Worked example B · trap repair</span><h4>${esc(trapPrompt)}</h4><div class="solution">
      <div class="step"><span class="step-num">1</span><div><b>Locate the fragile assumption.</b><br>${esc(p3)}</div></div>
      <div class="step"><span class="step-num">2</span><div><b>Stress-test it.</b><br>${esc(p5)}</div></div>
      <div class="step"><span class="step-num">3</span><div><b>Replace intuition with a checkable rule.</b><br>Write the prevention rule you would want in your Error Log, then apply it to the checkpoint below.</div></div>
    </div></div>
  </section>`
}
function renderTopicTraps(t){
  ...
  const base = t.module==='quant' ? [
    ['Hidden assumption','Treating a variable as positive, integer, nonzero, or a diagram as to-scale when the prompt never licensed that assumption.'],
    ['Over-computation','Doing exact arithmetic before simplifying, estimating, factoring, testing a boundary, or using answer choices.'],
    ['Illegal shortcut','Using a familiar rule outside its domain—for example squaring, reciprocating, canceling, averaging, or scaling without checking conditions.'],
    ['Wrong target','Correct mathematics aimed at the wrong quantity, unit, denominator, or comparison.']
  ] : t.module==='verbal' ? [
    ['Choice-first reading','Letting an attractive word or familiar phrase dictate the sentence/passage before deriving the logic.'],
    ['Strength inflation','Choosing always/entirely/proves when the passage supports may/often/suggests.'],
    ['Viewpoint or scope swap','Assigning a cited view to the author, or expanding a claim from one group/time frame to another.'],
    ['Topic match','Choosing an option because it discusses the same subject while performing the wrong logical job.']
  ] : t.module==='writing' ? [
    ['Generic template','Answering the topic but not the exact instruction.'],
    ['Name-dropping','Listing examples without explaining the warrant or mechanism that makes them evidence.'],
    ['False sophistication','Adding length or rare vocabulary instead of analytical precision.'],
    ['Token counterargument','Mentioning the other side without stating the condition under which it actually matters.']
  ] : [
    ['Hero-question syndrome','Spending excessive time on one item while easier points remain unseen.'],
    ['Rule without evidence','Following a pacing or review habit because it sounds smart rather than because timed data supports it.'],
    ['Anxiety switching','Changing an answer or strategy with no specific new evidence.'],
    ['Score-only review','Tracking mock scores without identifying the process that produced misses and slow correct answers.']
  ];
  const topicSpecific=[];
  if(pts[2])topicSpecific.push(['Chapter-specific pressure point',pts[2]]);
  if(pts[4])topicSpecific.push(['Edge case to remember',pts[4]]);
  return `<section class="lesson-section"><span class="eyebrow">Trap radar</span><h2>What usually breaks</h2><div class="trap-grid">${[...base,...topicSpecific].map(x=>`<div class="trap"><strong>${esc(x[0])}</strong><p>${esc(x[1])}</p></div>`).join('')}</div></section>`
}
function renderPracticeLadder(t){
  const pts=deepPoints(t); if(!pts.length)return '';
  const cues = t.module==='quant' ? [
    'State the controlling rule and the exact domain in which it is valid.',
    'Construct one legal counterexample to the most tempting false assumption.',
    'Explain how the GRE could disguise this skill inside a word problem, QC item, or data display.',
    'Name the fastest valid method and one slower method you should avoid.',
    'Identify a boundary value, sign case, unit issue, or diagram assumption that could flip an answer.',
    'Write a one-line Error Log rule that would prevent your most likely miss.'
  ] : t.module==='verbal' ? [
    'State the logical/evidentiary task before looking at answer choices.',
    'Name the clue word, scope term, speaker, or structural signal most likely to control the answer.',
    'Invent one tempting wrong answer and classify its trap type.',
    'Paraphrase the chapter’s key rule in ordinary English with no GRE jargon.',
    'Explain the difference between a plausible answer and a textually licensed answer here.',
    'Write the exact behavior you will use when you are 50/50 under time.'
  ] : t.module==='writing' ? [
    'State the scoring dimension this chapter improves and what a weak essay does instead.',
    'Write a one-sentence thesis/body claim that demonstrates the chapter’s principle.',
    'Give a concrete example and then add the missing warrant beginning “This matters because…”',
    'Name one condition or counterexample that would force a qualification.',
    'Identify the highest-impact revision you would make with three minutes left.',
    'Write a one-line checklist item to use after every timed Issue essay.'
  ] : [
    'Turn the chapter into a specific if/then decision rule.',
    'Name the metric that would tell you whether the rule is helping.',
    'Describe a case where following the rule mechanically would be wrong.',
    'State the failure mode you would log after a bad timed set.',
    'Define the delayed retest that would count as evidence of mastery.',
    'Write the exact test-day cue you want to execute automatically.'
  ];
  return `<section class="lesson-section"><span class="eyebrow">Six-rep mastery workout</span><h2>Do not mark this chapter complete from recognition alone</h2><p class="section-intro">Answer each prompt aloud or on scratch paper before opening the model note. These are retrieval/transfer drills, not the cold multiple-choice checkpoint that follows.</p><div class="retrieval-grid">${cues.map((cue,i)=>`<details class="retrieval-card"><summary><span>${i+1}</span>${esc(cue)}</summary><div><b>Model note</b><p>${esc(pts[i]||pts[0])}</p></div></details>`).join('')}</div><div class="practice-cta"><div><b>Need more reps?</b><p>Use the Drill Engine for randomized questions, then log every miss or lucky guess. Official ETS/POWERPREP remains the calibration source.</p></div><button class="primary-btn" data-route="drill">Open Drill Engine →</button></div></section>`
}
...
function renderLesson(t){
  ...
  return `<div class="page lesson-layout"><article class="lesson-main" data-topic-id="${t.id}">
    <header class="lesson-hero"><div class="crumbs"><button data-route="${t.module}">${mod.name}</button><span>›</span><span>${idx+1} / ${topics.length}</span></div><h1>${esc(t.title)}</h1><p>${esc(t.summary)}</p><div class="pill-row"><span class="badge ${t.module==='quant'?'red':t.module==='verbal'?'blue':t.module==='writing'?'':'gold'}">${esc(t.difficulty)}</span><span class="badge">~${lessonTimeRange(t)} min with practice</span><span class="badge green">${deepPoints(t).length} deep mastery notes</span>${done?'<span class="badge green">Mastered ✓</span>':''}</div></header>
    ...
    ${renderTopicTraps(t)}
    ${renderPracticeLadder(t)}
    ${t.quiz?.length?`<section class="lesson-section cold-check"><span class="eyebrow">Cold GRE checkpoint · not used above</span><h2>Now solve a different question</h2><p class="section-intro">Do this without reopening the worked examples. The purpose is to test transfer rather than recognition.</p>${t.quiz.map((q,i)=>renderQuiz(q,t.id,i)).join('')}</section>`:''}
    <section class="lesson-section"><h2>Mastery standard</h2><div class="checklist"><div class="checkline">I can explain the central rule and its restrictions without looking at this page.</div><div class="checkline">I can name the common GRE trap and construct a counterexample or evidence check.</div><div class="checkline">I can solve a fresh problem accurately without copying the worked examples.</div><div class="checkline">I can recognize the skill when mixed with another topic or disguised by surface wording.</div><div class="checkline">I can still do it after a delay and under realistic timing.</div></div></section>
    <div class="route-footer">${prev?`<button data-route="${prev.id}">← ${esc(prev.title)}</button>`:'<button data-route="'+t.module+'">← Module home</button>'}${next?`<button data-route="${next.id}">${esc(next.title)} →</button>`:'<button data-route="drill">Drill this module →</button>'}</div>
  </article>
  <aside class="lesson-aside">
    <div class="aside-card"><span class="eyebrow">Chapter status</span><h4>${done?'Mastered':'In progress'}</h4><p>Mark complete only after the cold checkpoint and retrieval workout are genuinely reproducible.</p><button id="completeTopic" class="complete-btn ${done?'done':''}" data-topic="${t.id}">${done?'✓ Mastered — click to undo':'Mark mastered +25 XP'}</button></div>
    ...
    <div class="aside-card note-box"><span class="eyebrow">Your note</span><h4>Memory hook</h4><p>Store only the rule, trap, or counterexample future-you is likely to forget.</p><textarea id="topicNote" data-topic="${t.id}" placeholder="e.g. restriction / trap / one-line rule…">${esc(note)}</textarea></div>
    <div class="aside-card"><span class="eyebrow">Official anchor</span><h4>Source of truth</h4><p>GRE Atlas teaches the skill. ETS defines the current exam, formats, scoring, and policy.</p><a href="${t.module==='quant'?OFFICIAL.quant:t.module==='verbal'?OFFICIAL.verbal:t.module==='writing'?OFFICIAL.writing:OFFICIAL.tips}" target="_blank" rel="noopener">Open ETS guidance ↗</a></div>
  </aside></div>`
}
function paragraphize(s){return String(s).split(/\n\n+/).map(x=>`<p>${esc(x)}</p>`).join('')}
function topicCallout(t){
  const text=t.module==='quant'?'Before computing, ask what the problem is really testing—structure, sign, scale, constraints, or arithmetic.':t.module==='verbal'?'Evidence and logic beat intuition. Predict first, then force every answer choice to prove itself.':t.module==='writing'?'The essay is scored holistically. Develop a precise position and reasoning chain; ornate vocabulary cannot rescue shallow analysis.':'Strategy is useful only when it protects expected points. Build rules from timed evidence, not superstition.';
  return `<div class="callout"><b>Perfect-score lens</b>${esc(text)}</div>`
}
function renderQuiz(q,tid,idx){
  const [text,opts,ans,exp]=q; return `<div class="quiz-card" data-quiz="${tid}-${idx}" data-answer='${JSON.stringify(ans)}'><div class="q">${esc(text)}</div><div class="choices">${opts.map((o,i)=>`<button class="choice" data-i="${i}">${String.fromCharCode(65+i)}. ${esc(o)}</button>`).join('')}</div><div class="explanation"><b>Why:</b> ${esc(exp)}</div></div>`
}
...
function buildTodayPlan(){
  ...
  const first=pending[0]||TOPICS[0], second=pending.find(t=>t.module!==first.module)||pending[1]||TOPICS[1];
  const day=new Date().getDay();
  return [
    {icon:'Aa',title:'Vocabulary retrieval',sub:'20 due words + 5 unseen',time:'15m',route:'vocab'},
...
function genQuant(kind){
  ...
  if(kind==='data'){const jan=ri(40,90),feb=jan+ri(5,30),mar=feb-ri(0,20),ans=((feb-jan)/jan)*100;return {label:'Data interpretation',text:`A table reports monthly output: January ${jan}, February ${feb}, March ${mar}. By approximately what percent did output increase from January to February?`,answer:+ans.toFixed(2),tolerance:.15,explain:`Percent increase=(${feb}−${jan})/${jan}×100≈${ans.toFixed(2)}%.`}}
  const bank=[
...
function buildStudyEntries(){
  ...
  return [...map.values()].sort((a,b)=>(rank[a.tier]??9)-(rank[b.tier]??9)||a.word.localeCompare(b.word));
}
...
async function lookupDictionary(word){
  const key=vocabKey(word);if(state.vocabDefs?.[key]?.definition)return state.vocabDefs[key];
...
function renderMocks(){
 ...
 ${rows.length?`<div class="section-title"><div><span class="eyebrow">Trend</span><h2>What the numbers say</h2></div></div><div class="stat-strip"><div class="stat-cell"><b>${Math.max(...rows.map(r=>r.q||130))}</b><span>best Quant</span></div><div class="stat-cell"><b>${Math.max(...rows.map(r=>r.v||130))}</b><span>best Verbal</span></div><div class="stat-cell"><b>${Math.max(...rows.map(r=>r.a||0)).toFixed(1)}</b><span>best AWA</span></div><div class="stat-cell"><b>${rows.length}</b><span>simulations</span></div></div>`:''}
...
function renderFormula(){
  const groups=['quant','verbal','writing','strategy'].map(m=>({m,label:MODULES[m].name,topics:byModule(m)}));
  return `<div class="page"><header class="route-header"><div><span class="eyebrow">Whole-test quick reference</span><h1>Formula & Framework Atlas</h1><p>Quant has literal formulas; Verbal, Writing, and Strategy have equally important decision frameworks. Every one of the ${TOPICS.length} chapters now has its own memorization deck, collected here for rapid review.</p></div><div class="route-stamp"><div><b>ƒ</b><small>${TOPICS.length}<br>chapter decks</small></div></div></header><div class="formula-grid">${FORMULAS.map(f=>`<section class="formula-card"><h3>${f[0]}</h3><ul>${f[1].map(x=>`<li><span class="formula">${esc(x)}</span></li>`).join('')}</ul></section>`).join('')}</div><div class="callout"><b>Use this correctly</b>Formula recognition is not enough. For each line, know the trigger, restrictions, and one GRE trap. Open the linked chapter whenever a framework is not instantly retrievable.</div><div class="section-title"><div><span class="eyebrow">All subjects</span><h2>Chapter-by-chapter formula decks</h2></div><p>Collapsed by default so this remains a usable reference instead of a wall of text.</p></div><div class="toolkit-index">${groups.map(g=>`<section class="card toolkit-index-group"><div class="coverage-head"><div><span class="eyebrow">${esc(g.label)}</span><h3>${g.topics.length} chapters</h3></div><span class="badge">${g.topics.length*4}+ formulas / frameworks</span></div>${g.topics.map(t=>{const k=topicToolkit(t);return `<details class="formula-topic-detail"><summary><span>${esc(t.title)}</span><button type="button" class="text-link" data-route="${t.id}">Open chapter ↗</button></summary><div class="formula-topic-body"><div><b>Tricks</b><ul>${k.tricks.map(x=>`<li>${esc(x)}</li>`).join('')}</ul></div><div><b>${esc(toolkitLabels(t)[1])}</b><ul class="formula-mini-list">${k.formulas.map(x=>`<li><code>${esc(x)}</code></li>`).join('')}</ul></div></div></details>`}).join('')}</section>`).join('')}</div></div>`
}
...
function bindQuiz(card){
  ...
  const choices=$$('.choice',card);let picked=[];
  choices.forEach(c=>c.onclick=()=>{
    ...
    if(Array.isArray(answer)){
      ...
      if(picked.length===answer.length){
        ...
        $('.explanation',card).classList.add('show');card.dataset.done='1';state.xp=(state.xp||0)+(ok?5:1);saveState();toast(ok?'+5 XP · correct':'Checkpoint reviewed');
      }
    } else {
      const ok=i===answer;c.classList.add(ok?'correct':'wrong');choices[answer]?.classList.add('correct');$('.explanation',card).classList.add('show');card.dataset.done='1';state.xp=(state.xp||0)+(ok?5:1);saveState();toast(ok?'+5 XP · correct':'Checkpoint reviewed');
    }
  })
}
function bindRoadmap(){
  $('#savePlan').onclick=()=>{state.examDate=$('#examDateInput').value;state.hoursWeek=+$(`#hoursWeekInput`).value||10;state.targetQ=+$(`#targetQ`).value||170;state.targetV=+$(`#targetV`).value||170;state.targetAWA=+$(`#targetA`).value||6;saveState();toast('Roadmap saved');render()}
}
function bindDiagnostic(){
  if($('#resetDiag')){$('#resetDiag').onclick=()=>{state.diag=null;diagIndex=0;diagAnswers=[];saveState();render()};return}
  $$('.diag-option').forEach(b=>b.onclick=()=>{
    const d=DIAGNOSTIC[diagIndex],pick=+b.dataset.diag;let ok;
    if(Array.isArray(d.a)){const chosen=[pick];ok=d.a.length===1&&d.a[0]===pick; /* multi handled below */
      if(d.a.length>1){if(!window.__diagMulti)window.__diagMulti=[];b.classList.toggle('selected');window.__diagMulti=b.classList.contains('selected')?[...window.__diagMulti,pick]:window.__diagMulti.filter(x=>x!==pick);if(window.__diagMulti.length<d.a.length)return;ok=[...window.__diagMulti].sort().join(',')===[...d.a].sort().join(',');window.__diagMulti=[]}
    } else ok=pick===d.a;
    diagAnswers.push({i:diagIndex,ok,skill:d.skill,m:d.m});
    if(diagIndex<DIAGNOSTIC.length-1){diagIndex++;render()}else{const q=diagAnswers.filter(x=>x.m==='quant'&&x.ok).length,v=diagAnswers.filter(x=>x.m==='verbal'&&x.ok).length,total=q+v;const weak=diagAnswers.filter(x=>!x.ok).map(x=>x.skill);state.diag={q,v,total,weak:[...new Set(weak)].slice(0,8),date:Date.now()};state.xp=(state.xp||0)+20;saveState();render()}
  })
}
function bindDrill(){
...
function finishDrill(ok,explain,picks,answers){drillState.answered=true;drillState.total++;if(ok)drillState.correct++;state.xp=(state.xp||0)+(ok?5:1);saveState();if(picks){$$('.drill-choice').forEach(b=>{const i=+b.dataset.i;if(answers.includes(i))b.classList.add('correct');else if(picks.includes(i))b.classList.add('wrong')})}const box=$('#drillFeedback');box.className=`feedback ${ok?'good':'bad'}`;box.innerHTML=`<b>${ok?'Correct.':'Not quite.'}</b><br>${explain}<div style="margin-top:9px"><button id="nextDrill" class="pill-btn">Next question →</button>${!ok?'<button id="logDrillError" class="pill-btn">Log this miss</button>':''}</div>`;$('#nextDrill').onclick=()=>{newDrillQuestion();render()};if($('#logDrillError'))$('#logDrillError').onclick=()=>{state.errors.unshift({text:`Drill miss: ${drillState.q.label} — ${drillState.q.text}`.slice(0,240),type:'Reasoning gap',module:drillState.module==='quant'?'Quant':'Verbal',date:Date.now()});saveState();toast('Added to Error Log')}}
function bindVocab(route='vocab'){
  ...
  if($('#flashcard'))$('#flashcard').onclick=async()=>{
    ...
    if(vocabFlipped&&word){const e=findStudyEntry(word),info=e?entryDefinition(e):null;if(e&&!info?.definition){await lookupDictionary(word);if(currentRoute()==='vocab'&&vocabFlipped)render()}}
  };
...
function bindEssay(){
  $('#essayEditor').oninput=e=>{state.essayDraft=e.target.value;saveState();const n=e.target.value.trim()?e.target.value.trim().split(/\s+/).length:0;$('#essayWords').textContent=n+' words'};
  $('#newPrompt').onclick=()=>{state.essayPrompt=(state.essayPrompt+1)%ESSAY_PROMPTS.length;state.essayDraft='';essaySeconds=1800;clearInterval(essayTimer);essayTimer=null;saveState();render()};
  ...
  $('#essayStart').onclick=()=>{if(essayTimer){clearInterval(essayTimer);essayTimer=null;toast('Essay timer paused')}else{essayTimer=setInterval(()=>{essaySeconds=Math.max(0,essaySeconds-1);const el=$('#essayTimer');if(el)el.textContent=fmtTime(essaySeconds);if(essaySeconds===0){clearInterval(essayTimer);essayTimer=null;toast('30 minutes — stop writing and revise what you can see.')}},1000)}};
...
function bindErrors(){
 $('#addError').onclick=()=>{const text=$('#errorText').value.trim();if(!text)return toast('Describe the error first');state.errors.unshift({text,type:$('#errorType').value,module:$('#errorModule').value,date:Date.now()});saveState();toast('Error logged');render()};
...
function bindSettings(){
 ...
 $('#importData').onchange=e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=()=>{try{const x=JSON.parse(r.result);state={...defaultState,...x};saveState();toast('Backup imported');render()}catch{toast('Could not read that backup')}};r.readAsText(f)};
 $('#resetAll').onclick=()=>{if(confirm('Reset ALL GRE Atlas progress, notes, vocabulary scheduling, errors, and mocks in this browser?')){localStorage.removeItem(STORAGE_KEY);state={...defaultState};saveState();routeTo('dashboard');render()}}
}

let focusSeconds=1500,focusTimer=null;
function openFocus(){focusSeconds=focusSeconds||1500;$('#focusModal').hidden=false;$('#focusTime').textContent=fmtTime(focusSeconds)}
function closeFocus(){if($('#focusModal'))$('#focusModal').hidden=true}
function initFocus(){
 $('#focusBtn').onclick=openFocus;$$('[data-close-focus]').forEach(x=>x.onclick=closeFocus);$('#focusClose').onclick=closeFocus;
 $('#focusStart').onclick=()=>{if(focusTimer){clearInterval(focusTimer);focusTimer=null;$('#focusStart').textContent='Resume'}else{$('#focusStart').textContent='Pause';focusTimer=setInterval(()=>{focusSeconds=Math.max(0,focusSeconds-1);$('#focusTime').textContent=fmtTime(focusSeconds);if(focusSeconds===0){clearInterval(focusTimer);focusTimer=null;$('#focusStart').textContent='Start';toast('Focus block complete · +10 XP');state.xp=(state.xp||0)+10;saveState()}},1000)}};
...
function openSearch(){const m=$('#searchModal');m.hidden=false;const i=$('#globalSearch');i.value='';renderSearch('');setTimeout(()=>i.focus(),20)}
function closeSearch(){$('#searchModal').hidden=true}
function renderSearch(term){term=term.trim().toLowerCase();const scored=SEARCH_INDEX.map(x=>{const hay=(x.title+' '+x.sub+' '+x.terms).toLowerCase();let score=term?term.split(/\s+/).reduce((s,w)=>s+(hay.includes(w)?1:0),0):1;if(term&&x.title.toLowerCase().includes(term))score+=3;return {...x,score}}).filter(x=>x.score>0).sort((a,b)=>b.score-a.score).slice(0,12);$('#searchResults').innerHTML=scored.map(x=>`<div class="search-result" data-search-route="${x.route}"><div><strong>${x.title}</strong><small>${x.sub}</small></div><span>↵</span></div>`).join('')||'<div class="empty-state">No matching chapter.</div>';$$('[data-search-route]').forEach(x=>x.onclick=()=>{closeSearch();routeTo(x.dataset.searchRoute)})}
function initSearch(){
 $('#searchTrigger').onclick=openSearch;$('#globalSearch').oninput=e=>renderSearch(e.target.value);$$('[data-close-modal]').forEach(x=>x.onclick=closeSearch);
 document.addEventListener('keydown',e=>{if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='k'){e.preventDefault();openSearch()}if(e.key==='Escape'){closeSearch();closeFocus()}})
}
function initChrome(){
...
function updateStreak(){const last=state.lastStudy?new Date(state.lastStudy+'T00:00:00'):new Date();const now=new Date(todayISO()+'T00:00:00');const diff=Math.round((now-last)/86400000);if(diff===1){state.streak=(state.streak||1)+1;state.lastStudy=todayISO();saveState()}else if(diff>1){state.streak=1;state.lastStudy=todayISO();saveState()}else if(diff===0){}else{state.lastStudy=todayISO();saveState()}}

window.addEventListener('hashchange',()=>{try{render()}catch(e){console.error('GRE Atlas render error',e);const a=$('#app');if(a)a.innerHTML=`<div class="page empty-state"><h3>Something went wrong displaying this page.</h3><p>${esc(String(e&&e.message||e))}</p><button class="pill-btn" data-route="dashboard">Back to dashboard</button></div>`}});
document.addEventListener('DOMContentLoaded',()=>{
  const step=fn=>{try{fn()}catch(e){console.error('GRE Atlas startup step failed; continuing.',e)}};
  step(updateStreak);step(updateChrome);step(initChrome);step(initSearch);step(initFocus);
  step(()=>{if(!safeStorage.persistent())toast('Progress is using temporary in-memory storage in this browser session.')});
  step(()=>{if(!location.hash)location.hash='#/'+(state.lastRoute||'dashboard');else render()});
